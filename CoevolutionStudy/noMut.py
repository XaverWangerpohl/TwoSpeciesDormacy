...
U0 = V0 / (U_out / U_in)
Y0 = Yss
size = 10000  # scaling to integer counts
...
model birth_death()
  ...
  Y_symbiosis:    => Y; Y_birth/size * (W) * Y
  ...
  W_compete: W => ; W_birth/(size^2) * W * ( W) * Y
  Y_compete: Y => ; Y_birth/(size^2) * Y * ( W) * Y
end
"""
...
def plot_VWY_trajectories(npz_file: str = 'results_all.npz',
    ...
    # indices for V, W, Y
    idxs = [species.index(s) for s in ('W','Y')]
    ...
    for sp,i in zip(('W','Y'), idxs):
        ...
        if sp == 'W':
            w_extinct = int(extinct)

    title = ', '.join(f"{label_map.get(sp, sp)}: {pct[sp]:.1f}%" for sp in pct)
    # Count how many W paths went extinct
    if 'w_extinct' not in locals():
        # Fallback if loop didn't set it (e.g., missing 'W')
        w_idx = species.index('W') if 'W' in species else None
        w_extinct = int(np.sum((all_data[:,:,w_idx]==0).any(axis=1))) if w_idx is not None else 0
    title_text = fr"\textbf{{Trajectories for}} $\mathbf{{K_{{pop}}={size}}}$\textbf{{, {w_extinct/n_reps*100:.1f}\% extinct}} "
    ...
    out = os.path.join(output_dir,f'VWY_trajectories_{size}_2species.png')
...
def main():
    ...
    p.add_argument('--t-end',    type=float, default=10000.0)
